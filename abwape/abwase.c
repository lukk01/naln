...
void abwase_bwa_cal_pac_pos(const bntseq_t *bns, bwt_t *bwt, int n_seqs, bwa_seq_t *seqs, int max_mm, float fnr)
{
...
int abwase_core(char *prefix, char *fn_fa, gap_opt_t *opt, int n_occ) {
  ...
  { // load BWT
    ...
    strcpy(str, prefix); strcat(str, ".bwt");  bwt = bwt_restore_bwt(str);
    // following line is from bwa_cal_pac_pos (commented out in abwase_bwa_cal_pac_pos)
    strcpy(str, prefix); strcat(str, ".sa");   bwt_restore_sa(str,bwt);
    free(str);
  }
  ...
  while ((seqs = bwa_read_seq(ks, 0x40000, &n_seqs, opt->mode, opt->trim_qual)) != 0) {
    ...
    // duplicate reads
    // take a copy of seqs
    seqsC = abwape_copy_seqs(seqs,n_seqs,opt);
    
    // do alignment
    fprintf(stderr, "[bwa_aln_core] calculate SA coordinate ... ");
    abwape_bwa_aln_core_inner(bwt, seqsC, n_seqs,opt);
    ...
    */
    ...
    abwase_bwa_cal_pac_pos(bns, bwt, n_seqs, seqs, opt->max_diff, opt->fnr); // forward bwt will be destroyed here                                              
    fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
...
int print_bwase_usage(gap_opt_t *opt) {
  fprintf(stderr, "\n");
  fprintf(stderr, "Program:  abwase combines \'aln\' and \'samse\' from bwa (version %s) into one command.\n",PACKAGE_VERSION);
  fprintf(stderr, "Version:  %s\n",BWASE_VERSION);
  fprintf(stderr, "Contact:  Margus Lukk <margus.lukk@cruk.cam.ac.uk>\n\n");
  fprintf(stderr, "Usage:    abwase [options] <prefix> <in1.fq>\n");
  fprintf(stderr, "Options:  aln, samse followed by options as entered in bwa\n");
  fprintf(stderr, "Examples: abwase hg19.fa in1.fq in2.fq\n\n");
  fprintf(stderr, "          abwase aln \"-e 2\" bwase \"-n 30\" hg19.fa in1.fq\n\n");
...
int abwase(int argc, char *argv[]) {
  ...
  abwase_core(argv[j],argv[j+1],opt,n_occ);
