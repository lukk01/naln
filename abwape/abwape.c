...
#include <pthread.h>
#endif
// additional values from bwtaln.c
#include <stdint.h>
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
#endif
#ifndef BWAPE_VERSION
#define BWAPE_VERSION "0.1"
#endif

/* ******* FOLLOWING FUNCTIONS HAVE BEEN COPIED UNCHANGED FROM bwape.c ******* */
...
*/
/* ******* END OF FUNCTIONS COPIED UNCHANGED FROM  bwape.c ******* */

/* ******* FOLLOWING FUNCTION ADAPTED FROM  bwtaln.c ******* */

void abwape_bwa_aln_core_inner(bwt_t *bwt, bwa_seq_t *seqs, int n_seqs, const gap_opt_t *opt)
{
  // adapted from bwa_aln_core in bwtaln.c. The code is (unchanged) from the core loop of "while((seqs = bwa_read_seq ..."
  // bwa - the bwa index (as bwt)    
  // seqs - containing reads
  // n_seqs - containing the number of sequences held by seqs
  // opts - options
  // does not return anything but updates the sequences in seqs by adding number
  // of alignments seqs->n_aln and alignments seqs->aln
  ...
  bwa_cal_sa_reg_gap(0, bwt, n_seqs, seqs, opt);
#endif
  // alignments have been added to the seqs.
}

/* ******* END OF FUNCTION(S) ADAPTED FROM  bwtaln.c ******* */

int print_bwape_usage(gap_opt_t *opt,pe_opt_t *popt) {

  fprintf(stderr, "\n");
  fprintf(stderr, "Program:  abwape combines \'aln\' and \'sampe\' from bwa (version %s) into one command.\n",PACKAGE_VERSION);
  fprintf(stderr, "Version:  %s\n",BWAPE_VERSION);
  fprintf(stderr, "Contact:  Margus Lukk <margus.lukk@cruk.cam.ac.uk>\n\n");
  fprintf(stderr, "Usage:    abwape [options] <prefix> <in1.fq> <in2.fq>\n");
  fprintf(stderr, "Options:  aln, samse, sampe followed by options as entered in bwa\n");
  fprintf(stderr, "Examples: abwape hg19.fa in1.fq in2.fq\n\n");
  fprintf(stderr, "          abwape aln \"-e 2\" bwape \"-a 300 -o 2\" hg19.fa in1.fq in2.fq\n\n");
  ...
  fprintf(stderr, "\n");
  fprintf(stderr, "Options for \'sampe\' (bwa version %s):\n\n",PACKAGE_VERSION);
...
int parse_sampe(pe_opt_t *popt,char *bwa_rg_line,char *bwa_rg_id, char *args) {
  int c;
  extern int bwa_set_rg(const char *s);
...
bwa_seq_t *abwape_copy_seqs(bwa_seq_t *seqsFrom,int n_seqs,gap_opt_t *opt) {
  // Function returns copy sequences, quality values and name from input bwa_seq_t array.
  // NB! No other content in of bwa_seq_t container is copied.

  bwa_seq_t *seqsTo;
  int i,j;
  // int is_comp = opt->mode&BWA_MODE_COMPREAD, is_64 = opt->mode&BWA_MODE_IL13, l_bc = opt->mode>>24;

  seqsTo = (bwa_seq_t*)calloc(n_seqs, sizeof(bwa_seq_t));

  for(i=0; i < n_seqs; i++) {
    bwa_seq_t *p = seqsFrom + i;
    bwa_seq_t *p1 = seqsTo + i;

    // install some values
    p1->bc[0] = 0;
    p1->tid = -1; // no assigned to a thread
    p1->qual = 0;
    
    p1->full_len = p1->clip_len = p1->len = p->full_len;
    // add sequence
    p1->seq = (ubyte_t*)calloc(p->full_len, 1);
    for (j = 0; j != p1->full_len; ++j) // copy sequence
      p1->seq[j] = p->seq[j];
    // add quality
    p1->qual = (ubyte_t*)strdup((const char*)p->qual);
    
    // add reverse sequence                                                                                                    
    p1->rseq = (ubyte_t*)calloc(p1->full_len, 1);
    memcpy(p1->rseq, p->rseq, p1->len);
    p1->name = strdup((const char*)p->name);
...
int abwape_bwa_cal_pac_pos_pe(const bntseq_t *bns, const char *prefix, bwt_t *const _bwt, int n_seqs, bwa_seq_t *seqs[2] , bwa_seq_t *seqsAln[2], isize_info_t *ii, const pe_opt_t *opt, const gap_opt_t *gopt, const isize_info_t *last_ii) {
  // The function has been modified from bwa_cal_pac_pos_pe in bwape.c.
  // The original code took file handlers for sai files from which the alignment results were read to bwa_seq_t containers.
  // The modified code takes alignments in bwa_seq_t containers directly.
  ...
  for (i = 0; i != n_seqs; ++i) {
    ...
    for (j = 0; j < 2; ++j) {
      ...
      d->aln[j].n = n_aln;
      // fread(d->aln[j].a, sizeof(bwt_aln1_t), n_aln, fp_sa[j]);                                                                                                                                                                                             
      // d->aln[j].a = pAln[j]->aln                                                                                                                                                                                                                           
      memcpy(d->aln[j].a,pAln[j]->aln,sizeof(bwt_aln1_t) * n_aln);
    ...
    if (opt->N_multi || opt->n_multi) {
      for (j = 0; j < 2; ++j) {
        if (p[j]->type != BWA_TYPE_NO_MATCH) {
          int k, n_multi;
          if (!(p[j]->extra_flag&SAM_FPP) && p[1-j]->type != BWA_TYPE_NO_MATCH) {
            bwa_aln2seq_core(d->aln[j].n, d->aln[j].a, p[j], 0, p[j]->c1+p[j] \
                             ->c2-1 > opt->N_multi? opt->n_multi : opt->N_multi);
          } else bwa_aln2seq_core(d->aln[j].n, d->aln[j].a, p[j], 0, opt->n_multi);
          for (k = 0, n_multi = 0; k < p[j]->n_multi; ++k) {
            int strand;
            bwt_multi1_t *q = p[j]->multi + k;
...
int abwape_core(char *prefix, char *fn_fa1, char *fn_fa2, gap_opt_t *opt, pe_opt_t *popt) {
  // Function has been modified from bwa_sai2sam_pe_core in bwape.c.
  ...
  if (!(opt->mode & BWA_MODE_COMPREAD)) {
    ...
    // if (popt->is_preload) {
      fprintf(stderr,"[abwape] Loading bwt index ...");
      strcpy(str, prefix); strcat(str, ".bwt"); bwt = bwt_restore_bwt(str);
      strcpy(str, prefix); strcat(str, ".sa"); bwt_restore_sa(str, bwt);
      pac = (ubyte_t*)calloc(bns->l_pac/4+1, 1);
      rewind(bns->fp_pac);
  ...
  // initialization of file handlers
  ks[0] = bwa_open_reads(opt->mode, fn_fa1);
  ks[1] = bwa_open_reads(opt->mode, fn_fa2);
  ...
  while(1) {
    ...
    int n_seqs2;
    // read sequences
    seqs[0] = bwa_read_seq(ks[0], 0x40000, &n_seqs, opt->mode, opt->trim_qual);
    seqs[1] = bwa_read_seq(ks[1], 0x40000, &n_seqs2, opt->mode, opt->trim_qual);

    // check if sequences OK
    if(n_seqs != n_seqs2) {
      fprintf(stderr,"[abwape] Different number of reads in files %s and %s\n",fn_fa1, fn_fa2);
      break;
    }
    if(n_seqs == 0) break;

    // take a copy of seqs
    seqsC[0] = abwape_copy_seqs(seqs[0],n_seqs,opt);
    seqsC[1] = abwape_copy_seqs(seqs[1],n_seqs,opt);
    
    // align
    
    t = clock();
    fprintf(stderr, "[bwa_aln_core] calculate SA coordinate 1 ... ");
    abwape_bwa_aln_core_inner(bwt, seqsC[0], n_seqs,opt);
    fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();

    fprintf(stderr, "[bwa_aln_core] calculate SA coordinate 2 ... ");
    abwape_bwa_aln_core_inner(bwt, seqsC[1], n_seqs,opt);
    fprintf(stderr, "%.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();

    fprintf(stderr, "[bwa_sai2sam_pe_core] convert to sequence coordinate... \n");
    cnt_chg = abwape_bwa_cal_pac_pos_pe(bns, prefix, bwt, n_seqs, seqs, seqsC, &ii, popt, opt, &last_ii);
    fprintf(stderr, "[bwa_sai2sam_pe_core] time elapses: %.2f sec\n", (float)(clock() - t) / CLOCKS_PER_SEC); t = clock();
    fprintf(stderr, "[bwa_sai2sam_pe_core] changing coordinates of %d alignments.\n", cnt_chg);
    ...
    for (j = 0; j < 2; ++j) {
      bwa_free_read_seq(n_seqs, seqs[j]);
      bwa_free_read_seq(n_seqs, seqsC[j]);
    }
    tot_seqs += n_seqs;
    fprintf(stderr, "[bwa_sai2sam_pe_core] %d sequences have been processed.\n", tot_seqs);
    last_ii = ii;
...
int abwape(int argc, char *argv[]) {
  ...
  for(i = 1; i < argc; i++ ) {
	...
	parse_aln(opt,argv[i]);
      ...
      else print_bwape_usage(opt,popt);
      continue;
    }
    if(strcmp(argv[i],"sampe") == 0) {
      if( (i+1) < argc) {
	j=i+2;
	i++;
	parse_sampe(popt,bwa_rg_line,bwa_rg_id,argv[i]);
      }
      else print_bwape_usage(opt,popt);
      continue;
    }
  ...
  if(j+3 != argc) {
    print_bwape_usage(opt,popt);
    return 1;
  }

  // aln + sampe
  abwape_core(argv[j],argv[j+1],argv[j+2],opt,popt);
